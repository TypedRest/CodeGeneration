    ...
    where TEndpoint : IEndpoint
{
    ...
    public (JvmChildEndpoint child, IEnumerable<IJvmType> types) Build(string key, TEndpoint endpoint, IEndpointGenerator generator)
    {
        ...
        if (endpoint.Children.Count > 0 || RequiresGeneratedClass)
        {
            ...
            var creation = new JvmNew(memberType) {Arguments = {JvmThis.Instance}};
            if (endpoint.Uri == null) creation.Arguments.Add(RelativeUri(endpoint));
            value = creation;
        }

        return (
            new JvmChildEndpoint(generator.Naming.Property(key), memberType, value) {Summary = endpoint.Description},
            types);
    }
    ...
    {
        var implementation = new JvmEndpointClass(generator.EndpointType(key, endpoint))
        {
    ...
    /// </summary>
    /// <remarks>
    /// Most JVM endpoints take a <c>Class&lt;T&gt;</c> for the entity they deserialize, because TypedRest for the JVM avoids reified type parameters so that its endpoints stay usable from Java.
    /// </remarks>
