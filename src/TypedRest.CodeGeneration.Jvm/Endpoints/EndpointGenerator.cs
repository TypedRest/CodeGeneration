...
public class EndpointGenerator(INamingStrategy namingStrategy, BuilderRegistry builders, TypeNameRegistry? typeNames = null) : IEndpointGenerator
{
    ...
    public IEnumerable<IJvmType> Generate(EntryEndpoint endpoint)
    {
        ...
        // to read first. Its member carries the identifiers its generated types were given: with interfaces that is
        // both an interface and the class implementing it, which stay adjacent and in that order.
        MoveToFront(types, child.ImplementationType);
        MoveToFront(types, child.Type);

        return types;
    }

    private static void MoveToFront(List<IJvmType> types, JvmIdentifier identifier)
    {
        int index = types.FindIndex(x => x.Identifier.QualifiedName == identifier.QualifiedName);
        if (index <= 0) return;
