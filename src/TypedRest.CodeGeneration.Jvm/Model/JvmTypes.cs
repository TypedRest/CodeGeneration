...
public sealed class JvmEndpointClass(JvmIdentifier identifier) : IJvmType
{
    ...
    /// <summary>
    /// The generated interface this class implements, or <c>null</c> when interfaces are not being generated.
    /// </summary>
    ...
    public IEnumerable<JvmIdentifier> GetImports()
    {
        ...
        if (Interface != null)
        {
            foreach (var package in Interface.GetImports()) yield return package;
        }
...
/// </summary>
/// <param name="identifier">The name of the interface and the package it is declared in.</param>
/// <remarks>Generated only when interfaces are turned on, declaring the same children as the class implementing it.</remarks>
public sealed class JvmEndpointInterface(JvmIdentifier identifier) : IJvmType
{
    /// <inheritdoc/>
    public JvmIdentifier Identifier { get; } = identifier;
    ...
    /// <summary>
    /// The TypedRest endpoint interface this one extends, or <c>null</c> for an endpoint kind that has none.
    /// </summary>
    ...
    public IEnumerable<JvmIdentifier> GetImports()
    {
        ...
        // Only the declared types matter here; an interface declares its children without constructing them
        foreach (var package in Children.SelectMany(x => x.Type.GetImports())) yield return package;
    }
}
...
public sealed class JvmChildEndpoint(string name, JvmIdentifier type, JvmExpression value)
{
    ...
    /// <summary>
    /// The concrete type <see cref="Value"/> constructs, which is what an element factory has to instantiate.
    /// </summary>
    /// <remarks>Differs from <see cref="Type"/> only when interfaces are being generated.</remarks>
    public JvmIdentifier ImplementationType { get; set; } = type;
