...
public static class Packages
{
    ...
    /// </summary>
    public static JvmIdentifier EntryEndpoint { get; } = new(Endpoints, "EntryEndpoint");
    ...
    /// </summary>
    /// <remarks>
    /// <c>Endpoint</c> itself is an interface and there is no <c>EndpointImpl</c>, so unlike every other kind the
    /// plain endpoint has no concrete class to instantiate. <c>AbstractEndpoint</c> is abstract, which is why
    /// <see cref="Endpoints.DefaultBuilder"/> always generates a class rather than constructing one inline.
    /// </remarks>
    public static JvmIdentifier AbstractEndpoint { get; } = new(Endpoints, "AbstractEndpoint");

    /// <summary>
    /// Returns the implementation class a generated endpoint derives from.
    /// </summary>
    public static JvmIdentifier Implementation(JvmPackage package, string name)
        => new(package, name + "Impl");
}
