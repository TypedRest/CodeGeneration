...
public sealed class JvmIdentifier
{
    ...
    /// <summary>
    /// The simple name of the interface this type implements, or <c>null</c> if it implements none.
    /// </summary>
    /// <remarks>The interface lives in the same package and takes the same type arguments.</remarks>
    public string? InterfaceName { get; private set; }

    /// <summary>
    /// The interface this type implements, or <c>null</c> if it implements none.
    /// </summary>
    public JvmIdentifier? Interface
    {
        get
        {
            if (InterfaceName is null) return null;

            var result = new JvmIdentifier(Package, InterfaceName, Nullable) {Kind = Kind};
    ...
    {
        var result = new JvmIdentifier(Package, name, Nullable) {Kind = Kind, InterfaceName = InterfaceName};
        result.TypeArguments.AddRange(TypeArguments);
        return result;
    }

    /// <summary>
    /// Returns a copy of the type reference that implements the interface <paramref name="name"/>.
    /// </summary>
