...
public sealed class JavaWriter(JvmSerializer serializer, bool entryConstructor = true, bool nullableAnnotations = true)
{
    ...
    private IEnumerable<JvmIdentifier> AllImports(IJvmType type)
    {
        ...
        {
            // The entry endpoint's constructor takes only the base URI; every other one takes a referrer as well
    ...
    private void WriteEndpoint(JvmWriter writer, JvmEndpointClass type)
    {
        ...
        string extends = $" extends {TypeExpression(type.BaseType)}"
                       + (type.Interface is {} generatedInterface ? $" implements {TypeExpression(generatedInterface)}" : "");
        ...
        using (writer.Indent())
        {
            ...
            {
                writer.WriteLine();
                WriteChild(writer, child, type.Interface != null);
            }
        }
    ...
    private void WriteInterface(JvmWriter writer, JvmEndpointInterface type)
    {
        ...
        string extends = type.BaseType is {} baseType ? $" extends {TypeExpression(baseType)}" : "";
        writer.WriteLine($"public interface {type.Identifier.Name}{extends} {{");
        ...
        using (writer.Indent())
        {
            ...
            foreach (var child in type.Children)
            {
                ...
                writer.WriteLine($"{TypeExpression(child.Type)} {Accessor(child)}();");
                first = false;
            }
        }
    ...
    /// <summary>
    /// The name of the accessor method for a child endpoint, following the JavaBean convention.
    /// </summary>
    private static string Accessor(JvmChildEndpoint child)
        => "get" + char.ToUpperInvariant(child.Name[0]) + child.Name.Substring(1);
    ...
    /// <summary>
    /// Writes a child endpoint as a getter over a <c>private final</c> field.
    /// </summary>
    /// <param name="writer">The writer to write to.</param>
    /// <param name="child">The child endpoint to write.</param>
    /// <param name="implementsInterface">Whether the class implements a generated interface, which declares the same getter and so makes this an override.</param>
    private void WriteChild(JvmWriter writer, JvmChildEndpoint child, bool implementsInterface)
    {
        writer.WriteLine($"private final {TypeExpression(child.Type)} {child.Name} =");
        using (writer.Indent())
            writer.WriteLine(Expression(child.Value) + ";");

        writer.WriteLine();
        writer.WriteDocComment(child.Summary, child.Deprecated);
        if (implementsInterface) writer.WriteLine("@Override");
        if (child.Deprecated) writer.WriteLine("@Deprecated");
        writer.WriteLine($"public {TypeExpression(child.Type)} {Accessor(child)}() {{");
        using (writer.Indent())
            writer.WriteLine($"return {child.Name};");
        writer.WriteLine("}");
    }
