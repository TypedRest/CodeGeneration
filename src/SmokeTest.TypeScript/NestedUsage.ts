...
import {NestedClient} from "./generated/nested";
import {TeamElementEndpoint} from "./generated/nested/TeamElementEndpoint";
import {ProjectElementEndpoint} from "./generated/nested/ProjectElementEndpoint";
import {Team} from "./generated/nested/dtos/Team";
...
export async function syncAndReadTeams(client: NestedClient): Promise<Team[]> {
  await client.teams.sync.invoke();
  return await client.teams.readAll();
}

export function teamElement(client: NestedClient, id: string): TeamElementEndpoint {
  // The generated element class, not the built-in ElementEndpoint
  return client.teams.get(id);
}

export async function archiveTeam(client: NestedClient, team: Team): Promise<void> {
  await client.teams.get(team).archive.invoke();
}

export async function createTeam(client: NestedClient, team: Team): Promise<TeamElementEndpoint | undefined> {
  return await client.teams.create(team);
}

export function projectElement(client: NestedClient, id: string): ProjectElementEndpoint {
  return client.projects.get(id);
}

export async function searchAndRename(client: NestedClient, id: string): Promise<void> {
